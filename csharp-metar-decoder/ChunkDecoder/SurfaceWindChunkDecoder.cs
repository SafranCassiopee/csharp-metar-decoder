...
namespace csharp_metar_decoder.chunkdecoder
{
    public sealed class SurfaceWindChunkDecoder : MetarChunkDecoder
    {
        public const string SurfaceWindParameterName = "SurfaceWind";
        ...
        {
            return $"^{DirectionRegexPattern}{SpeedRegexPattern}{SpeedVariationsRegexPattern}{UnitRegexPattern}{DirectionVariationsRegexPattern}( )";
        ...
        public override Dictionary<string, object> Parse(string remainingMetar, bool withCavok = false)
        {
            ...
            if (found.Count <= 1)
            {
                throw new MetarChunkDecoderException(remainingMetar, newRemainingMetar, MetarChunkDecoderException.Messages.SurfaceWindInformationBadFormat, this);
            }

            // handle the case where nothing is observed
            if (found[1].Value == "///" && found[2].Value == "//")
            {
                throw new MetarChunkDecoderException(remainingMetar, newRemainingMetar, MetarChunkDecoderException.Messages.NoSurfaceWindInformationMeasured, this);
            }
            
            Value.Unit speedUnit = Value.Unit.None;
            // get unit used
            switch (found[5].Value)
            {
                case "KT":
